...
async function fetchAllIssues(
    ...
    while (hasMore) {
        ...
        outputChannel.appendLine(`Fetched ${data.issues.length} issues (total: ${allIssues.length}/${data.total})`);
    ...
    const severityCount = allIssues.reduce((acc, issue) => {
        acc[issue.severity] = (acc[issue.severity] || 0) + 1;
        return acc;
    }, {} as Record<string, number>);
    outputChannel.appendLine(`Original SonarQube severity distribution: ${JSON.stringify(severityCount)}`);
...
async function convertToAnalysisResult(
    ...
    for (const ruleKey of uniqueRules) {
        try {
            ...
            outputChannel.appendLine(`Warning: Failed to fetch rule details for ${ruleKey}`);
        }
    }
...
export async function exportIssues(context: vscode.ExtensionContext): Promise<void> {
    const outputChannel: vscode.OutputChannel =
        (global as any).gooseOutputChannel ||
        vscode.window.createOutputChannel("Goose SonarQube");
    ...
    try {
        ...
        if (!connectionTest.success) {
            vscode.window.showErrorMessage(
                `Failed to connect to SonarQube: ${connectionTest.error}`
            );
            return;
        }
        ...
        await vscode.window.withProgress(
            ...
            async (progress) => {
                ...
                const filteredFileAnalyses = analysisResult.fileAnalyses.map((fileAnalysis) => ({
                    ...fileAnalysis,
                ...
                if (totalFilteredIssues === 0) {
                    vscode.window.showInformationMessage("No issues match the selected Software Quality types.");
                ...
                const severityFilteredFileAnalyses = filteredResult.fileAnalyses.map((fileAnalysis) => ({
                    ...fileAnalysis,
                    issues: fileAnalysis.issues.filter((issue) =>
