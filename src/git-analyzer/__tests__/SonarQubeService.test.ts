...
// Mock fs operations for report-task.txt
vi.mock("fs", () => ({
  existsSync: vi.fn((path: string) => {
    // Return true for report-task.txt paths
    return path.includes("report-task.txt");
  }),
  readFileSync: vi.fn((path: string) => {
    if (path.includes("report-task.txt")) {
      return "ceTaskId=mock-task-id\ndashboardUrl=http://localhost:9000/dashboard?id=test-project\n";
    }
    return "";
  }),
}));
...
vi.mock("sonarqube-scanner", () => ({
  default: vi.fn((config, callback) => {
    ...
    // Scanner callback signature: callback(error?)
    // When error is undefined/null, scanner succeeded
    setTimeout(() => {
      if (mockScannerShouldFail) {
        // Call callback with error
        callback(new Error("Scanner execution failed"));
      } else {
        // Call callback with no error (undefined) to indicate success
        callback(undefined);
      }
    }, 10);
  }),
...
describe("SonarQubeService", () => {
  ...
  describe("getAnalysisResult", () => {
    ...
    it("should handle API errors gracefully", async () => {
      ...
      const issuesUrl =
        "http://localhost:9000/api/issues/search?componentKeys=test-project&resolved=false&ps=500";
  ...
  describe("waitForAnalysis", () => {
    ...
    it("should throw error if server is not available", async () => {
      ...
      await expect(unavailableService.waitForAnalysis("task-123")).rejects.toThrow("not available");
    });
